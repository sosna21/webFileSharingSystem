...
export class UploadButtonsComponent {
  ...
  uploadSelectedFiles($event: Event) {
    ...
    const filesWithPath: { file: File; path: string }[] = files.map(file => ({ file: file, path: '' }));
    this.fileUploadService.uploadFiles(directories, filesWithPath, this.fileService.parentId()).subscribe();
  ...
  uploadSelectedFolder($event: Event) {
    ...
    const filesWithPath: { file: File; path: string }[] = files.map(file => ({ file: file, path: file.webkitRelativePath }));
    this.fileUploadService.uploadFiles(directories, filesWithPath, this.fileService.parentId()).subscribe();
  }
  ...
  private getDirectoriesFromFiles(files: File[]): { path: string }[] {
    const directories: { path: string }[] = [];
    files.forEach(file => {
      const dirs = file.webkitRelativePath.split('/').slice(0, -1).join('/').concat('/');
