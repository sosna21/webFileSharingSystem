...
namespace webFileSharingSystem.Web.Controllers
{
    public class UploadController: BaseController
    {
        ...
        public UploadController(IUnitOfWork unitOfWork, ICurrentUserService currentUserService, IFilePersistenceService filePersistenceService)
        {
        ...
        public async Task<ActionResult<PartialFileInfo>> UploadFileChunkAsync(int fileId, int chunkIndex, [FromForm] IFormFile chunk)
        {
            var userId = _currentUserService.UserId;
        ...
        public async Task<ActionResult<PartialFileInfo>> CompleteFileUploadAsync(int fileId)
        {
            ...
            if (!partialFileInfo.PersistenceMap.CheckIfAllBitsAreZeros()) return BadRequest("Not all chunks were uploaded correctly to the server");
