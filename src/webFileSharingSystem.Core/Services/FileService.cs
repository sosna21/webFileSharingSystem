...
public class FileService : IFileService
{
    ...
    public async Task<(Result<OperationResult>, IEnumerable<FilePathPart>?)> GetPathToFileAsync(int fileId, int userId, CancellationToken cancellationToken = default)
    {
        ...
        var filePathParts = await _unitOfWork.CustomQueriesRepository().FindPathToAllParents(fileId, cancellationToken);
    ...
    public async Task<Result<OperationResult>> DeleteDirectoryAsync(int directoryFileId, int userId,
        CancellationToken cancellationToken = default)
    {
        var directoryToDelete = await _unitOfWork.Repository<File>().FindByIdAsync(directoryFileId, cancellationToken);
        ...
        foreach (var guidFilesCount in fileGuidsToRemove.Where(g => g.Count == 0))
        {
    ...
    public async Task<Result<OperationResult>> MoveFilesAsync(int? newParentId, IEnumerable<int> fileIds, int userId, CancellationToken cancellationToken = default)
    {
        var newOwnerUserId = userId;
        if (newParentId is not null)
        {
            var parentFile = await _unitOfWork.Repository<File>().FindByIdAsync(newParentId.Value, cancellationToken);
            if (parentFile is null) return Result.Failure(OperationResult.BadRequest, "Parent directory not found");
            if (!parentFile.IsDirectory) return Result.Failure(OperationResult.BadRequest, "Parent file is not a directory");
    ...
    public async Task<Result<OperationResult>> CopyFilesAsync(int? newParentId, IEnumerable<int> fileIds, int userId,
        CancellationToken cancellationToken = default)
    {
        var newOwnerUserId = userId;
        if (newParentId is not null)
